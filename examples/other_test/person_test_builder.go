// Code generated by BuilderGen v0.4.0
package other

import (
	"github.com/Jh123x/buildergen/examples/nested"
	"github.com/Jh123x/buildergen/examples"
)

type PersonBuilder struct {
	ID        int
	Name      string
	Email     *string
	PhoneBook []*examples.Contact
	MapVal    map[string]string   `json:"map_val"`
	T         nested.Test
}

func NewPersonBuilder(b *examples.Person) *PersonBuilder {
	if b == nil {
		return nil
	}

	return &PersonBuilder{
		ID:        b.ID,
		Name:      b.Name,
		Email:     b.Email,
		PhoneBook: b.PhoneBook,
		MapVal:    b.MapVal,
		T:         b.T,
	}
}

func (b *PersonBuilder) WithID(iD int) *PersonBuilder {
	b.ID = iD
	return b
}

func (b *PersonBuilder) WithName(name string) *PersonBuilder {
	b.Name = name
	return b
}

func (b *PersonBuilder) WithEmail(email *string) *PersonBuilder {
	b.Email = email
	return b
}

func (b *PersonBuilder) WithPhoneBook(phoneBook []*examples.Contact) *PersonBuilder {
	b.PhoneBook = phoneBook
	return b
}

func (b *PersonBuilder) WithMapVal(mapVal map[string]string) *PersonBuilder {
	b.MapVal = mapVal
	return b
}

func (b *PersonBuilder) WithT(t nested.Test) *PersonBuilder {
	b.T = t
	return b
}

func (b *PersonBuilder) Build() *examples.Person {
	return &examples.Person{
		ID:        b.ID,
		Name:      b.Name,
		Email:     b.Email,
		PhoneBook: b.PhoneBook,
		MapVal:    b.MapVal,
		T:         b.T,
	}
}
